...
Adds ability to EXCLUDE columns by substring (case-insensitive) via config
or CLI (--exclude).
...
# ------------------------------
# CONFIG: header substrings to exclude (case-insensitive)
# Any header that contains one of these substrings will be ignored
EXCLUDE_COLUMN_SUBSTRINGS = [
    # example excludes - edit as needed:
    "practice",
    "notes",
    "address",
    # add more substrings to ignore
]
# ------------------------------
...
def collect_degree_values_from_folder(
    ...
    # normalize exclude substrings
    if exclude_list is None:
        exclude_list = []
    exclude_norm = [e.casefold().strip() for e in exclude_list if e and e.strip()]
    ...
    for f in files:
        ...
        for sheet in sheets:
            ...
            # but skip headers that match EXCLUDE substrings
            matched_cols = []
            for col in headers:
            ...
            for col in matched_cols:
                ...
                for actual_col in df_sheet.columns:
                    # skip actual columns that are excluded
                ...
                if found_col_name is None:
                    # try substring match among actual columns (skip excluded actual columns)
                    for actual_col in df_sheet.columns:
                        if any(exc in str(actual_col).casefold() for exc in exclude_norm):
                            continue
                        if any(k in str(actual_col).casefold() for k in cand_norm) and str(col).casefold() in str(actual_col).casefold():
                            ...
                            break
                if found_col_name is None:
                    # fallback to the candidate col string itself (but still check exclusion)
...
def main():
    ...
    parser.add_argument("--exclude", type=str, default=None, help="Comma-separated list of header substrings to exclude (overrides config).")
    ...
    df_res = collect_degree_values_from_folder(input_folder, cand_list, scan_rows=args.scan_rows, recursive=args.recursive, exclude_list=exclude_list)
